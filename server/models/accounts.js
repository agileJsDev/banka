...
class Accounts {
  ...
  static async findAccountByNo(accountnumber) {
    const data = await pool.query('SELECT * FROM accounts WHERE accountnumber = $1', [
      accountnumber
    ]);
    if (data.rowCount < 1) return false;
    return data.rows[0];
  ...
  static async updateAccountStatus(accountNumber, newStatus) {
    const { rows } = await pool.query(
      'UPDATE accounts SET status = $1, modifieddate = $2 WHERE accountnumber = $3 RETURNING *',
  ...
  static async debit(accountNumber, amount) {
    ...
    const { rows } = await pool.query(
      'UPDATE accounts SET balance = $1, modifieddate = $2 WHERE accountnumber = $3 RETURNING *',
