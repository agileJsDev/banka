...
class Transactions {
  ...
  static async getUserTransactions(accountNumber) {
    const data = await pool.query('SELECT * FROM transactions WHERE accountnumber = $1', [
      accountNumber
    ]);
  ...
  static async findByID(id) {
    const data = await pool.query('SELECT * FROM transactions WHERE transactionid = $1', [id]);
    if (data.rowCount < 1) return false;
