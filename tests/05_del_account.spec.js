  ...
  describe('DELETE /api/v1/accounts/<account_number>', () => {
    describe('When Admin/Staff decides to delete an account', () => {
      ...
      it('Send unauthorized message if user not authorized', async () => {
        const res = await chai.request(app).delete('/api/v1/accounts/1111111111').set('Authorization', userToken);
      ...
      it('should respond with error 404 - not found - if account number does not exist', async () => {
        const res = await chai.request(app).delete('/api/v1/accounts/1111111111').set('Authorization', adminToken);
      ...
      it('should respond with error 401 - unauthorized - if token is invalid or expired', async () => {
        const res = await chai.request(app).delete(`/api/v1/accounts/${account[0].accountNumber}`).set('Authorization', 'eyJhbGciOiJIUzI1NiIsIkpXVCJ9.kcvIkS9ACezPO2DgAi-XvEikLy9ZA2y0kWiQ');
      ...
      it('should respond with an OK status code 200 if account is successfully deleted', async () => {
        const res = await chai.request(app).delete(`/api/v1/accounts/${account[0].accountNumber}`).set('Authorization', adminToken);
